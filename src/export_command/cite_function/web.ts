    ...
    return async function addWebCite(): Promise<void> {
        ...
        try {
            ...
            if (error instanceof Error) {
                vscode.window.showErrorMessage(i18n("error-with-name", lang, error.name, error.message));
            } else {
                vscode.window.showErrorMessage(i18n("error-add-web-site", lang, JSON.stringify(error)));
            }
        }
        finally {
            barMessage.dispose();
